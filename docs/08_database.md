...
При первом запуске контейнера БД выполняются SQL-скрипты из папки `db/`, подключённой как `/docker-entrypoint-initdb.d`. Скрипты создают таблицы и заполняют тестовые данные (локации и визиты).
